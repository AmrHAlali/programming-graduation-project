...
import { ExtendedEdge, useSafeAreaInsetsStyle, colors, hp } from "@/core";
...
interface BaseScreenProps {
  ...
  /**
   * When true, the ScrollView will automatically scroll to top when the children change.
   */
  scrollToTopOnChildrenChange?: boolean;
  /**
   * A value to watch; when it changes the ScrollView will scroll to top. Useful for step/ tab changes.
   */
...
function ScreenWithScrolling(props: ScreenProps): JSX.Element {
  ...
  // detect children identity changes
  const childrenKey = useMemo(() => {
    // try to derive a stable key from children; fallback to JSON length/hash
    try {
      return Array.isArray(children)
        ? children.map((c: any) => c?.type?.name || c?.type || String(c)).join("|")
        : (children as any)?.type?.name || String(children);
    } catch {
      return String(children);
    }
  }, [children]);

  // Scroll when children change (optional) or when a trigger value changes
  const trigger = (props as any).scrollToTopTrigger;
  useEffect(() => {
    const shouldByChildren = (props as any).scrollToTopOnChildrenChange;
    const shouldByTrigger = typeof trigger !== "undefined";
    if (!shouldByChildren && !shouldByTrigger) return;

    const scroll = ref.current;
    if (scroll && typeof scroll.scrollTo === "function") {
      // small timeout to ensure layout updated
      const t = setTimeout(() => scroll.scrollTo({ x: 0, y: 0, animated: true }), 50);
